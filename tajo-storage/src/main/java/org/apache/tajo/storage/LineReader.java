...
public class LineReader implements Closeable {
  ...
  public int readLine(NonSyncByteArrayOutputStream str, ArrayList<Integer> offsets, int maxLineLength,
                      int maxBytesToConsume) throws IOException {
    if (this.recordDelimiterBytes != null) {
      return readCustomLine(str, offsets, maxLineLength, maxBytesToConsume);
    } else {
      return readDefaultLine(str, offsets, maxLineLength, maxBytesToConsume);
    }
  }
...
/*  int validIdx = 0;
   ...
   * Read a line terminated by a custom delimiter.
   */
  private int readCustomLine(NonSyncByteArrayOutputStream str, ArrayList<Integer> offsets, int maxLineLength, int maxBytesToConsume)
      throws IOException {
   /* We're reading data from inputStream, but the head of the stream may be
    *  already captured in the previous buffer, so we have several cases:
    *
    ...
    */
    ...
    do {
      ...
      if (bufferPosn >= bufferLength) {
        ...
        bufferLength = fillBuffer(in, buffer, ambiguousByteCount > 0);
        if (bufferLength <= 0) {
          str.write(recordDelimiterBytes, 0, ambiguousByteCount);
          break; // EOF
        }
      }
      for (; bufferPosn < bufferLength; ++bufferPosn) {
